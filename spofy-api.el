...
(declare-function spofy-auth-access-token-async "spofy-auth" (callback))
(declare-function spofy-auth-refresh-token "spofy-auth" (&optional callback))
  ...
  (let* ((full-url (if params (spofy-api--build-url url params) url))
    ...
    (if-let* ((cached (and (equal method "GET")
      ...
      (unless (spofy-api--rate-limit-active-p full-url)
        (spofy-auth-access-token-async
         (lambda (token)
           ;; Spotify requires a JSON body (even "{}") on PUT/POST/DELETE;
           ;; omitting it causes "Malformed json".
                            ...
                            (cond
                             ...
                             ((eql status-code 429)
                              (let ((delay (spofy-api--parse-retry-after)))
                                ...
                                (if (< retry-count spofy-api--max-retries)
