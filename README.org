...
The command ~spofy-library-browse-tracks~ displays the user's saved tracks in a =*Spofy Saved Tracks*= buffer.  Columns include a play indicator, track name, artist(s), album, and duration.  The currently playing track is highlighted, just like in search results.  When the library cache is warm ([[#h:cache-management][Cache management]]), all items render instantly; otherwise the command falls back to paginated API requests.  Column widths are controlled by the ~spofy-columns~ user option ([[#h:user-options-columns][Column widths]]).
