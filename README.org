...
The commands ~spofy-next~ and ~spofy-previous~ skip to the next or previous track, respectively.  Both commands issue an immediate re-poll on success so the UI reflects the new track without waiting for the next poll cycle.
...
- ~spofy-now-playing-track~, ~spofy-now-playing-artist~, and ~spofy-now-playing-album~ are used in the dashboard now-playing section.  They inherit from the corresponding base faces with a larger height (1.3×).
