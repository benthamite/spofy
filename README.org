...
The Spotify Web API can control playback on any device logged into the user's account: desktop apps, mobile phones, web players, smart speakers, and more.  The command ~spofy-select-device~ fetches the list of available devices asynchronously, presents them in a ~completing-read~ prompt when the response arrives, and transfers playback to the selected device.
