...
- *Column 2 — Spotify search, Browse, Library search, Other:* Spotify search for tracks (=s t=), albums (=s l=), artists (=s a=), playlists (=s p=), context tracks (=s c=); browse saved tracks (=b t=), saved albums (=b a=), playlists (=b p=), current context (=b c=); library search for saved tracks (=l t=), saved albums (=l a=), playlists (=l p=); devices (=d=), Wikipedia lookup (=w=), dashboard (=/=), quit (=q=).
...
The command ~spofy-library-browse-albums~ displays saved albums in a =*Spofy Saved Albums*= buffer with columns for name, artist(s), year, and track count.
...
The command ~spofy-search-context~ fetches the tracklist of the currently playing album or playlist and lets you jump to any track with incremental narrowing.  It is not available for Spotify-generated contexts such as radio or daily mixes.  This command is bound to =s c= in the transient popup ([[#h:transient-popup][Transient popup]]).
