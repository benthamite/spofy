...
The popup is organized into two columns:
...
- *Column 2 — Search, Browse, Library search, Other:* search tracks (=s t=), albums (=s l=), artists (=s a=), playlists (=s p=), current tracks (=s c=); browse saved tracks (=b t=), saved albums (=b a=), playlists (=b p=); library search for saved tracks (=l t=), saved albums (=l a=), playlists (=l p=); devices (=d=), Wikipedia lookup (=w=), dashboard (=/=), quit (=q=).
...
The command ~spofy-view-album~ displays the tracks of a Spotify album in a =*Spofy Album: NAME*= buffer.  The buffer has a header showing the album name, artist, year, and total track count.  Tracks are listed with their track number, name, artist(s), and duration.  For multi-disc albums, the track number column uses =disc.track= notation (e.g. =1.1=, =2.1=, =2.12=); single-disc albums show just the track number.  Column widths are controlled by the ~spofy-columns~ user option ([[#h:user-options-columns][Column widths]]).
...
#+findex: consult-spofy-my-track
#+findex: consult-spofy-my-album
#+findex: consult-spofy-my-playlist
#+findex: consult-spofy-device
#+findex: consult-spofy-context-track
...
The following three commands search within the user's own library rather than the full Spotify catalog.  They fetch all pages of the relevant endpoint on first use (paginating synchronously through the API), cache the results in memory, and filter client-side as you type.  The cache is invalidated automatically when items are saved or unsaved.  These are bound under the =l= prefix in the transient popup ([[#h:transient-popup][Transient popup]]).

The command ~consult-spofy-my-track~ searches the user's saved tracks with incremental narrowing.  Candidates show Name, Artist(s), Album, and Duration columns.  Selecting a track plays it immediately.

The command ~consult-spofy-my-album~ searches saved albums with incremental narrowing, with Name, Year, Artist(s), and number of tracks columns.  Selecting an album opens the album view ([[#h:album-view][Album view]]).

The command ~consult-spofy-my-playlist~ searches the user's playlists with incremental narrowing, with Name, Owner, and number of tracks columns.  Selecting a playlist opens the playlist detail view ([[#h:playlist-detail-view][Playlist detail view]]).
...
In addition to single-target actions, =spofy-embark.el= registers exporters for the =spofy-track=, =spofy-album=, and =spofy-playlist= categories.  When using a consult library search command (e.g., ~consult-spofy-my-track~), you can narrow the candidates, then call ~embark-export~ to export the matching candidates into a standard Spofy browse buffer with all the usual keybindings (play, view album, view artist, unsave, add to playlist, etc.).  This is useful when you want to filter your library and then explore or act on the results interactively rather than selecting a single item.
