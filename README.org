...
- *Playback tracking:* Jump to the currently playing track in any buffer, optional cursor-follows-playback mode, and a custom buffer mode-line showing playback state and track position ([[#h:tracking-playback][Tracking the playing position]]).
...
The now-playing section always appears at the top regardless of this option.  Each section is fetched asynchronously and includes a "View all" link that opens a full tabulated-list buffer.  Track and artist names in every section are truncated to the window width with a fade-out gradient when they would otherwise overflow.
...
| =.=   | ~spofy-jump-to-playing-track~ | Jump to playing track |
| =q=   | ~quit-window~             |
...
- *Column 1 — Playback, Volume, Mode:* play/pause, next, previous, seek forward, seek backward, seek to (=t=); volume up/down and set; shuffle and repeat toggles.
...
- *Column 3 — View and Other:* jump to playing track (=.=), cursor follows playback toggle (=F=); dashboard (=/=), devices (=d=), Wikipedia lookup (=w=), quit (=q=).
...
#+findex: spofy-jump-to-playing-track
#+findex: spofy-cursor-follows-playback-mode

When listening to a long playlist—especially in shuffle mode—it is easy to lose track of which item in the list is currently playing.  Spofy provides three complementary mechanisms to address this.

*** Jump to playing track
:PROPERTIES:
:CUSTOM_ID: h:jump-to-playing-track
:END:

#+findex: spofy-jump-to-playing-track
The command ~spofy-jump-to-playing-track~ navigates to the currently playing track.  It searches the current buffer first, then other open Spofy buffers, and finally opens the playing context (album or playlist) from the API if no buffer contains the track.  When the context must be fetched, the command uses the browse cache ([[#h:browse-cache][Browse cache]]) to avoid refetching previously loaded playlists.

The command is bound to =.= in all Spofy buffers—both tabulated-list views and the dashboard—via ~spofy-ui-set-format~.  It is also available in the transient popup ([[#h:transient-popup][Transient popup]]) under =.=.  After jumping, the echo area displays the track's position (e.g. "Spofy: track 47/213").
...
- Playback state icons: play/pause (⏸/▶), shuffle (⇌ when on), repeat (↻ for context, ↻₁ for track).
...
The mode-line updates automatically on player state changes (play/pause, shuffle, repeat) and on track changes.  The variable ~spofy-ui-mode-line-format~ holds the format list; it is set buffer-locally by ~spofy-ui-set-format~ in tabulated-list modes and by ~spofy-dashboard-mode~ in the dashboard.
...
The command ~spofy-list-recently-played~ displays the last 50 recently played tracks in a =*Spofy Recently Played*= buffer.  The columns are the same as saved tracks, and the playing highlight updates automatically when the track changes.
...
The first time a browse view is opened, all pages of items are fetched from the API and stored in the cache.  Subsequent opens—including those triggered by ~spofy-jump-to-playing-track~ ([[#h:jump-to-playing-track][Jump to playing track]])—serve items from the cache instantly, skipping the multi-page API fetch.  This is particularly noticeable for large playlists (hundreds or thousands of tracks) where the initial fetch may take several seconds.
...
The function ~spofy-ui-play-pause-icon~ returns a play or pause icon string based on the player state alist: =⏸= when playing, =▶= when paused.  The function ~spofy-ui-shuffle-indicator~ returns =⇌= when shuffle is on, or an empty string when off.  The function ~spofy-ui-repeat-indicator~ returns =↻= for context repeat, =↻₁= for track repeat, or an empty string when repeat is off.  These functions are used by the mode-line and tab-bar display modules.
