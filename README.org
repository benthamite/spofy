...
This manual describes the features and customization options for the Emacs Lisp package =spofy=.
...
- *Library management:* View and manage saved tracks and albums; follow and unfollow artists ([[#h:library-management][Library management]]).
- *Timeline:* A unified buffer showing recently played tracks, the now-playing track, and the upcoming queue in one view, with row actions for each ([[#h:timeline][Timeline]]).
...
- *Playback tracking:* Jump to the currently playing track in any buffer, optional cursor-follows-playback mode, and a custom buffer mode-line showing entity name, playback state, and track position ([[#h:tracking-playback][Tracking the playing position]]).
...
The development repository is on [[https://github.com/benthamite/spofy][GitHub]].
...
The user option ~spofy-poll-timeout~ sets the number of seconds spofy waits for an in-flight player state poll before treating it as stuck and scheduling another poll.  It defaults to ~15~.  Increase it only if your network routinely takes longer than that to return from Spotify; lower values recover faster from lost callbacks but may retry while a slow response is still in transit.
...
The user option ~spofy-tab-bar-format~ is a format string that controls what the tab-bar segment displays.  It defaults to ~"%t %a %s%r %G"~, which shows the track and artist, shuffle and repeat indicators, and a progress bar.  The same format specifiers as ~spofy-mode-line-format~ are supported ([[#h:user-options-mode-line][Mode-line]]), plus two additional specifiers: =%G= inserts a progress bar, and =%T= inserts the timestamps (e.g. "1:23 / 3:45").
...
#+vindex: spofy-dashboard-album-art
...
After successful authentication, spofy runs ~spofy-authenticated-hook~.  You only need to authenticate once; subsequent sessions reuse the stored refresh token to obtain new access tokens automatically.  Token exchange and refresh failures, including connections that close before returning a complete HTTP response, are reported without entering the debugger.  A failed refresh leaves the stored tokens unchanged.
...
The commands ~spofy-seek-forward~ and ~spofy-seek-backward~ move the playback position by ~spofy-seek-seconds~ seconds ([[#h:user-options-playback][Playback options]]).  The command ~spofy-seek-to~ prompts for an arbitrary timestamp and jumps to that position.  It accepts plain seconds (="90"=), =M:SS= (="1:30"=), or =H:MM:SS= (="1:05:30"=) format.  The prompt shows the track duration as a guide; the value is clamped to the range [0, duration].
...
#+findex: spofy-save-current-track
#+findex: spofy-unsave-current-track
#+findex: spofy-copy-url
...
:PROPERTIES:
:CUSTOM_ID: h:tracking-playback
:END:

#+findex: spofy-cursor-follows-playback-mode
...
:PROPERTIES:
:CUSTOM_ID: h:cursor-follows-playback
:END:
...
Enable it from the transient popup ([[#h:transient-popup][Transient popup]]) with =F=, or in your init file:
...
*** Buffer mode-line
:PROPERTIES:
:CUSTOM_ID: h:buffer-mode-line
:END:

#+vindex: spofy-ui-mode-line-format
...
The command ~spofy-player-start-polling~ starts a timer that polls the Spotify player state at ~spofy-poll-interval~ second intervals.  Polling is necessary because the Spotify Web API does not support push notifications.  The timer uses a self-rescheduling design: each poll is a one-shot timer that only schedules the next poll after the HTTP response callback completes.  This prevents request accumulation under network latency—a fixed-interval repeating timer would fire new requests even while previous ones were still in flight, causing unbounded buffer growth.  Each in-flight poll also has a recovery timer governed by ~spofy-poll-timeout~ ([[#h:user-options-playback][Playback options]]), so a lost callback cannot leave the dashboard, mode-line, or tab-bar stuck on stale player state.  A generation counter and per-request identifier ensure that stale callbacks from a previous polling loop do not accidentally reschedule after a stop, restart, or timeout, and spofy checks for stale timer objects when ensuring that polling is active.
...
The Spotify Web API can control playback on any device logged into the user's account: desktop apps, mobile phones, web players, smart speakers, and more.  The command ~spofy-select-device~ fetches the list of available devices asynchronously, presents them in a ~completing-read~ prompt when the response arrives, transfers playback to the selected device, and refreshes the cached player state so the dashboard, mode line, and tab bar update.

Playback commands never perform synchronous network requests.  Commands that need an active device first use the cached player state; if no device is known, they refresh state asynchronously and then prompt with ~spofy-select-device~ when needed.  Commands that require a current track, such as seek, volume, shuffle, repeat, and Wikipedia lookup, fail fast when no player state is available yet.

If device discovery fails because Spotify returns an API error, such as an authentication problem or rate limit response, spofy reports that API error instead of treating it as an empty device list.
...
When ~spofy-consult~ is loaded, these commands prompt for a query, fetch Spotify results asynchronously, and then present the returned candidates with Consult narrowing.  Without ~spofy-consult~, they prompt for a query string and display results in a tabulated list.
...
| =SPC= | ~spofy-play-pause~            | Toggle play/pause                   |
| =g=   | ~spofy-search-refresh~        | Re-run the search                   |
| =q=   | ~quit-window~                 | Close the buffer                    |

The Spotify API returns search results in pages.  Results load automatically as the user scrolls ([[#h:pagination][Pagination]]).
...
| =F=   | ~spofy-follow-artist~         | Follow this artist          |
| =U=   | ~spofy-unfollow-artist~       | Unfollow this artist        |
| =SPC= | ~spofy-play-pause~            | Toggle play/pause           |
| =g=   | ~spofy-artist-refresh~        | Refresh                     |
| =q=   | ~quit-window~                 | Close                       |

The function ~spofy-view-artist-top-tracks~ fetches the artist's top tracks (as determined by Spotify's algorithm) and displays them in a =*spofy Top Tracks: ARTIST*= buffer.  Tracks are listed with their name, album, and duration.  The currently playing track is highlighted and the highlight updates automatically when the track changes.  Column widths are controlled by the ~spofy-columns~ user option ([[#h:user-options-columns][Column widths]]).

| Key   | Command                              | Action                      |
|-------+--------------------------------------+-----------------------------|
| =RET= | ~spofy-artist-top-tracks-play~       | Play the track              |
| =a=   | ~spofy-artist-top-tracks-view-album~ | View album                  |
| =s=   | ~spofy-artist-top-tracks-save~       | Save track to library       |
| =SPC= | ~spofy-play-pause~                   | Toggle play/pause           |
| =g=   | ~spofy-artist-top-tracks-refresh~    | Refresh                     |
| =q=   | ~quit-window~                        | Close                       |
...
#+findex: spofy-playlist-view-follow
...
| =f=   | ~spofy-playlist-view-follow~       | Follow playlist                |
| =SPC= | ~spofy-play-pause~                 | Toggle play/pause              |
...
#+findex: spofy-library-browse-playlists
...
#+findex: spofy-library-browse-tracks
#+findex: spofy-library-browse-albums
#+findex: spofy-library-browse-context
...
#+findex: spofy-follow-artist
#+findex: spofy-unfollow-artist

The command ~spofy-library-browse-tracks~ displays the user's saved tracks in a =*spofy Saved Tracks*= buffer.  Columns include a play indicator, track name, artist(s), album, and duration.  The currently playing track is highlighted with the ~spofy-playing~ face across all columns, and the highlight updates automatically when the track changes.  When the library cache is warm ([[#h:cache-management][Cache management]]), all items render instantly; otherwise the command falls back to paginated API requests.  Column widths are controlled by the ~spofy-columns~ user option ([[#h:user-options-columns][Column widths]]).
...
The command ~spofy-library-browse-context~ opens the currently playing album or playlist in a standard browse buffer.  For album contexts it opens the album view ([[#h:album-view][Album view]]); for playlist contexts it opens the playlist detail view ([[#h:playlist-detail-view][Playlist detail view]]).  It signals an error when the context is unsupported (e.g., Spotify-generated radio or daily mixes).

See [[#h:timeline][Timeline]] for the recently-played and queue views, which live in a single buffer.
...
The command ~spofy-follow-artist~ follows the artist at point (in artist views) or prompts for an artist ID.  The command ~spofy-unfollow-artist~ is its counterpart.  Both are bound to =F= and =U= in the artist view ([[#h:artist-view][Artist view]]), and to =f= and =u= in the embark artist map ([[#h:embark-integration][Embark integration]]).

** Timeline
:PROPERTIES:
:CUSTOM_ID: h:timeline
:END:

#+findex: spofy-view-timeline
#+findex: spofy-add-to-queue
#+vindex: spofy-timeline-history-limit
...
| Key   | Command                                   | Action                          |
|-------+-------------------------------------------+---------------------------------|
| =RET= | ~spofy-timeline-play-at-point~            | Play the track at point         |
| =a=   | ~spofy-timeline-view-album-at-point~      | View album                      |
| =A=   | ~spofy-timeline-view-artist-at-point~     | View artist                     |
| =s=   | ~spofy-timeline-save-at-point~            | Save to library                 |
| =Q=   | ~spofy-timeline-queue-at-point~           | Add to queue                    |
| =SPC= | ~spofy-play-pause~                        | Toggle play/pause               |
| =g=   | ~spofy-view-timeline~                     | Refresh                         |
| =q=   | ~quit-window~                             | Close                           |
...
#+findex: spofy-library-warm-cache
#+findex: spofy-library-invalidate-cache
...
Saved track and saved album browse buffers ignore JSON null entries returned by Spotify, so a single unavailable saved item does not prevent the rest of the cached library from rendering.
...
*** Browse cache
:PROPERTIES:
:CUSTOM_ID: h:browse-cache
:END:

#+findex: spofy-browse--cache-get
#+findex: spofy-browse--cache-put
#+findex: spofy-browse--cache-invalidate
...
#+findex: spofy-player-interpolated-progress
...
#+findex: spofy-ui-entity-at-point
...
All four accept an endpoint path (relative to =https://api.spotify.com/v1/=), optional data, and a callback function that receives the parsed JSON response.  They handle authentication headers, asynchronous token refresh, rate limiting (429 responses), transient error retry, and response buffer cleanup automatically.  When Spotify returns HTTP 429, spofy records the =Retry-After= value as a shared cooldown and blocks non-cached API requests until that cooldown expires, before token lookup or network I/O.  If no valid access token is available, the request is skipped and a message suggesting =M-x spofy-authenticate= is shown instead of sending a malformed request.
...
#+findex: spofy-auth-access-token-async
#+findex: spofy-auth-refresh-token

The function ~spofy-auth-access-token~ returns the current OAuth2 access token, or ~nil~ if unavailable or expired.  Because the token expiry is persisted to disk alongside the tokens, this function can determine after an Emacs restart whether the stored access token is still valid without contacting Spotify.  It never refreshes synchronously.

The function ~spofy-auth-access-token-async~ calls a callback with a usable access token, refreshing asynchronously first when the current token is expired and a refresh token is available.

The function ~spofy-auth-refresh-token~ explicitly uses the stored refresh token to obtain a new access token from Spotify asynchronously.  It requires ~spofy-client-id~ and ~spofy-client-secret~ to be set and calls its optional callback with the refreshed token or ~nil~ on failure.
...
#+findex: spofy-ui-progress-bar-only
#+findex: spofy-ui-progress-time
#+findex: spofy-ui-insert-header
#+findex: spofy-ui-play-pause-icon
#+findex: spofy-ui-shuffle-indicator
#+findex: spofy-ui-repeat-indicator
...
:PROPERTIES:
:CUSTOM_ID: h:entity-storage-and-list-rendering
:END:

#+findex: spofy-ui-store-entity
#+findex: spofy-ui-entity-at-point
#+findex: spofy-ui-extract-id
#+findex: spofy-ui-album-year
#+findex: spofy-ui-playing-indicator
#+findex: spofy-ui-render-list
...
#+findex: spofy-library-search-tracks
#+findex: spofy-library-search-albums
#+findex: spofy-library-search-playlists
#+findex: consult-spofy-device
#+findex: spofy-library-search-context
...
The command ~consult-spofy-track~ prompts for a track query, fetches matching tracks asynchronously, and then lets you narrow the returned candidates.  Candidates show Name, Artist(s), Album, and Duration columns.  Selecting a track plays it immediately.

The command ~consult-spofy-album~ provides the same async fetch-then-narrow flow for albums, with Name, Year, Artist(s), and number of tracks columns.  Selecting an album opens the album view ([[#h:album-view][Album view]]).
...
The command ~consult-spofy-playlist~ searches Spotify playlists with async fetch-then-narrow behavior, with Name, Owner, and number of tracks columns.  Selecting a playlist opens the playlist detail view ([[#h:playlist-detail-view][Playlist detail view]]).

The following three commands search within the user's own library rather than the full Spotify catalog.  They rely on the library cache ([[#h:cache-management][Cache management]]), which is populated asynchronously in the background when spofy starts.  All saved items are available for instant client-side filtering as you type.  If the cache is not yet ready (e.g., at the start of a session), the command displays a message and starts the background fetch.  These are bound under the =l= prefix in the transient popup ([[#h:transient-popup][Transient popup]]).
...
The command ~spofy-library-search-albums~ searches saved albums with incremental narrowing, with Name, Year, Artist(s), and number of tracks columns.  Selecting an album opens the album view ([[#h:album-view][Album view]]).

The command ~spofy-library-search-playlists~ searches the user's playlists with incremental narrowing, with Name, Owner, and number of tracks columns.  Selecting a playlist opens the playlist detail view ([[#h:playlist-detail-view][Playlist detail view]]).

The command ~consult-spofy-device~ fetches available playback devices asynchronously and then presents them with Consult narrowing, with Name and Type columns.  Selecting a device transfers playback to it.

The command ~spofy-library-search-context~ fetches the tracklist of the currently playing album or playlist and lets you jump to any track with incremental narrowing.  It is not available for Spotify-generated contexts such as radio or daily mixes.  This command is bound to =l c= in the transient popup ([[#h:transient-popup][Transient popup]]).

The Spotify catalog search commands fetch up to 20 results for the initial query and then pass those candidates to ~consult--read~ for local narrowing.  This avoids blocking Emacs on network I/O while the minibuffer is active.  To browse further, use ~embark-export~ to send the candidates to a paginated search buffer with infinite scroll ([[#h:embark-integration][Embark integration]]).
...
#+findex: spofy-embark-export-tracks
#+findex: spofy-embark-export-albums
#+findex: spofy-embark-export-artists
#+findex: spofy-embark-export-playlists
...
#+findex: spofy-wikipedia-track
#+findex: spofy-wikipedia-album
#+findex: spofy-wikipedia-artist

Wikipedia lookup actions are available through Embark ([[#h:embark-integration][Embark integration]]) on tracks, albums, and artists (bound to =w= in each keymap).  The function ~spofy-wikipedia-track~ classifies the artist and dispatches to either the album or work lookup.  The function ~spofy-wikipedia-album~ performs a direct album lookup.  The function ~spofy-wikipedia-artist~ looks up the artist's Wikipedia page.  All three receive the Embark target string and extract entity metadata from its text properties.
...
#+vindex: spofy-now-playing-track
#+vindex: spofy-now-playing-artist
#+vindex: spofy-now-playing-album
...
When enabled, the mode inserts a ~tab-bar-format-spofy~ entry into ~tab-bar-format~ at a position determined by ~spofy-tab-bar-alignment~ ([[#h:user-options-tab-bar][Tab-bar options]]).  The segment is computed fresh on each tab-bar render, so it automatically adapts when other tab-bar elements appear or disappear.  Player state changes and theme changes trigger a re-render, and when the format string contains =%G= or =%T=, a 1-second timer keeps the progress bar updating smoothly.  When disabled, the mode removes the entry, stops the progress timer, and cleans up any ~tab-bar-format-align-right~ entry that was added automatically.
