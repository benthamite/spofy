...
- *Column 2 — Search, Browse, Other:* search tracks (=s t=), albums (=s l=), artists (=s a=), playlists (=s p=), current tracks (=s c=); browse saved tracks (=b t=), saved albums (=b a=), playlists (=b p=); devices (=d=), Wikipedia lookup (=w=), dashboard (=/=), quit (=q=).
